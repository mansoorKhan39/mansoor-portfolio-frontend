...
import { Trash2, Mail, MailOpen, Loader, RefreshCw, ArrowLeft } from 'lucide-react'
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-white mb-1">Messages</h1>
          <p className="text-slate-500 text-sm font-mono">
            {messages.length} total
            {unreadCount > 0 && <span className="ml-2 text-brand-400">{unreadCount} unread</span>}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <>
            ...
            {showDetail && selected ? (
              <div className="card">
                ...
                <div className="flex items-start justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
                  <div>
                    <h2 className="font-display font-bold text-white text-lg mb-1">{selected.subject || '(No subject)'}</h2>
                ...
                <p className="text-slate-300 font-body leading-relaxed whitespace-pre-wrap text-sm">{selected.message}</p>
              ...
              <div className="space-y-2">
                {messages.map(msg => (
                  <div key={msg._id} onClick={() => handleSelect(msg)}
                    className={`card cursor-pointer transition-all hover:border-slate-700 ${!msg.read ? 'border-brand-500/20' : ''}`}>
          ...
          <div className="hidden md:grid md:grid-cols-5 gap-6" style={{ height: 'calc(100vh - 220px)' }}>
            ...
            <div className="md:col-span-3">
              {selected ? (
                <div className="card h-full overflow-y-auto">
                  ...
                  <div className="text-slate-300 font-body leading-relaxed whitespace-pre-wrap">{selected.message}</div>
